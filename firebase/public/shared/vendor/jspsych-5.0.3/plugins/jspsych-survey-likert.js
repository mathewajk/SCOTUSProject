...
jsPsych.plugins['survey-likert'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    display_element.append('<form class="text-center" id="jspsych-survey-likert-form">');
    // add likert scale questions
    for (var i = 0; i < trial.questions.length; i++) {
      form_element = $('#jspsych-survey-likert-form');
      // add question
      form_element.append('<label class="jspsych-survey-likert-statement">' + trial.questions[i] + '</label><br/><br/>');
      // add options
      ...
      for (var j = 0; j < trial.labels[i].length; j++) {
        if(trial.polarities[i] == 0) {
          ...
          options_string += '<li style="width:' + width + '%"><input type="radio" name="Q' + i + '" value="' + (3-j) + '"><label class="jspsych-survey-likert-opt-label">' + trial.labels[i][j] + '</label></li>';
        }
      }
      options_string += '</ul><br/><br/>';
    ...
    $("#jspsych-survey-likert-next").html('Next');
